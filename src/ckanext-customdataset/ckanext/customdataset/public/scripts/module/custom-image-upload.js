 ...
 * 
 */  
this.ckan.module('custom-image-upload', function($, _) {
  return {
    ...
    options: {
      ...
      i18n: {
        ...
        url: _('Link'),
        remove: _('Remove'),
        invalidFileType: _('Invalid file type. File extension must be '),
    ...
    initialize: function () {
      ...
      this.fileExtension = this._getDatasetAllowedFileExtension();
    },
    ...
    _onInputChange: function() {
      ...
      if (this.fileExtension.length > 0){
            if (this.file_name.substr(this.file_name.length - this.fileExtension.length, this.fileExtension.length).toLowerCase() != this.fileExtension.toLowerCase()) {
                var errMessage = this.i18n('invalidFileType');
                $('.flash-messages').find('.alert-error').each(function()
                {
                    $(this).remove();
                })
    ...
    _getDatasetAllowedFileExtension: function(){
        var extension = "";
